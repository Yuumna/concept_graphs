...
#SBATCH --partition lmbdlc_gpu-rtx2080   # short: -p <partition_name>
#SBATCH --job-name UNet_w/o_token_discrete_small           # short: -J <job name>
...
# Running the job
start=`date +%s`
python train_merge.py --run_desc fourth_run --our_labels True  --model U-Net  --batch_size 64 --n_epoch 100 --log_freq 1
end=`date +%s`
runtime=$((end-start))
