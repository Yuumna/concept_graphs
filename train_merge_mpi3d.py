...
parser.add_argument('--experiment', default="mpi3d_16_16", type=str)
...
parser.add_argument('--dataset', default="mpi3d_real_complex", type=str)
parser.add_argument('--our_labels', type=str2bool, default=True)
...
class ContextUnet(nn.Module):
    def __init__(self, in_channels, n_feat = 256, n_classes=10, dataset="", type_attention="", token_folder="", pixel_size=64):
        ...
        #self.n_conv = 2 if token_folder else (12 if pixel_size == 48 else 7)
        if token_folder:
            if pixel_size == 28:
                self.n_conv = 2
            self.n_conv = 4
        else:
            self.n_conv = 7 if pixel_size == 28 else 16
...
def training(args):
    ...
    if model=="DiT":
        ...
        if token_folder:
            loaded_model = DiT_models['concept_DIT_1_mpi3d'](class_dropout_prob = args.dropout ,**kwargs)
        else:
            loaded_model = DiT_models['concept_DIT_2_mpi3d'](class_dropout_prob = args.dropout ,**kwargs)
    ...
    train_dataset = load_dataset_mpi3d.my_dataset(tf, num_samples, dataset, configs=configs["train"], training=True, alpha=alpha, remove_node=remove_node, our_labels=our_labels)
    ...
    for config in output_configs: 
        test_dataset = load_dataset_mpi3d.my_dataset(tf, n_sample, dataset, configs=config, training=False, test_size=test_size, our_labels=our_labels)
            ...
            if (ep + 1) % args.log_freq == 0 or ep >= (n_epoch - 5): 
                for test_config in output_configs: 
                    ...
                    if token_folder:
                        ...
                        if scheduler=="DDIM":
                            x_tok, x_gen_store = ddpm.sample_ddim(n_sample, c_gen, (in_channels, 16, 16), device)
                            ...
                            x_tok, x_gen_store = ddpm.sample(n_sample, c_gen, (in_channels, 16, 16), device, guide_w=0.0)
                            x_gen = vqgan.decode(x_tok[...,1:,1:])
                            x_gen = x_gen[0] if isinstance(x_gen, tuple) else x_gen  # if dino loss is included
                            x_gen = (x_gen + 1)/2
                    else:
                        if scheduler=="DDIM": 
                            ...
                            print(f"x_gen shape: {x_gen.shape}, x_gen_store shape: {x_gen_store.shape}")
