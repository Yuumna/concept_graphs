...
def training(args):
    ...
    save_dir = './output_old/'+dataset+'/'+model+ '/' + space + '/'+ label+ '/'+ experiment+'/'
    ...
    mask_transf = MaskTransformer(img_size=28, hidden_dim=in_channels, nclass=len(n_classes), depth=8, heads=8, mlp_dim=1040, dropout=0.0, codebook_size=256, qk_norm=True, ignore_attn_to_source=True)
    ...
    for ep in range(n_epoch):
        ...
        for x, c in pbar:
            ...
            optim.step()
        gradients_log[ep] = epoch_grads
        
        with open(f"{save_dir}gradients_epoch_{ep}.json", "w") as f:
            json.dump(epoch_grads, f, indent=4)
        ...
        with torch.no_grad():
            ...
            if (ep + 1) % 100 == 0 or ep >= (n_epoch - 5): 
                for test_config in output_configs: 
                    ...
                    if scheduler=="DDIM":
                        x_tok, x_gen_store = ddpm.sample_ddim(n_sample, c_gen, (in_channels ,7, 7), device)
                        ...
                        x_tok, x_gen_store = ddpm.sample(n_sample, c_gen, (in_channels ,7, 7), device, guide_w=0.0)
                        x_gen = vqgan.decode(x_tok[...,1:,1:])
                        x_gen = x_gen[0] if isinstance(x_gen, tuple) else x_gen  # if dino loss is included
                        x_gen = (x_gen + 1)/2
            ...
            if (ep + 1) % 100 == 0 or ep == (n_epoch - 1):
                ...
                model_path = f"{checkpoint_dir}ddpm_epoch_{ep+1}.pth"
                torch.save({
                    'epoch': ep + 1,
