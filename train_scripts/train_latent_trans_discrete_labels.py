...
def training(args):
    ...
    if not os.path.isdir(save_dir): os.makedirs(save_dir)
    mask_transf = MaskTransformer(img_size=28, hidden_dim=in_channels, nclass=len(n_classes), depth=12, heads=8, mlp_dim=1000, dropout=0.1, codebook_size=256, qk_norm=True, ignore_attn_to_source=True)
    ddpm = DDPM(nn_model=mask_transf, 
