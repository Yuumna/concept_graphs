...
class DDPM(nn.Module):
    ...
    def forward(self, x, c):
        ...
        pred_noise =  self.nn_model(x_t, _ts / self.n_T, c, return_latents= [])
        #print(f'pred_noise shape {pred_noise.shape}, noise: {noise.shape}')
    ...
    def sample(self, n_sample, c_gen, size, device, guide_w = 0.0):
        ...
        for i in range(self.n_T, 0, -1):
            ...
            eps = self.nn_model(x_i, t_is, _c_gen, return_latents = []) #, context_mask)
            x_i = (
...
def training(args):
    ...
    save_dir = './output_old/'+'new_transf/' + dataset+'/'+model+ '/' + space + '/'+ label+ '/'+ experiment+'/'
    ...
    mask_transf = VisionTransformerTime(input_size=8, depth=12, hidden_size=384,
                 patch_size=1, num_heads=6, num_concepts=3, 
                 in_channels=128, learn_sigma=False)
