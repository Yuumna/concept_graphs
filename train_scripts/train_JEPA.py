...
from Network.test import VisionTransformer
...
parser.add_argument("--sr_latents", type=int, default=[4,5])
...
class DDPM(nn.Module):
    def __init__(self, nn_model, betas, n_T, device, drop_prob=0.1, n_classes=256, flag_weight=0, l_t=[]):
    ...
    def forward(self, x, c, sr_latents):
        """
        Forward pass with discrete token diffusion.
        """
        _ts = torch.randint(1, self.n_T+1, (x.shape[0],)).to(self.device)  # t ~ Uniform(0, n_T)
        # at time step 1
        # we sample from the prior, so we don't need to add noise
        # to the input image
        t_1 = torch.tensor([1.0]).to(self.device)
        t_1 = t_1.repeat(x.shape[0], 1, 1, 1)
        ...
        pred_noise, l_latents =  self.nn_model(x_t, _ts / self.n_T, c, sr_latents)
        
        return self.loss_mse(noise, pred_noise), l_latents

    def sample(self, n_sample, c_gen, size, device, sr_latents ,guide_w = 0.0):
        ...
        for i in range(self.n_T, 0, -1):
            ...
            eps, _ = self.nn_model(x_i, t_is, _c_gen, sr_latents) #, context_mask)
            x_i = (
    ...
    def sample_ddim(self, n_sample, c_gen, size, device, sr_latents):
        """
        ...
        for i in reversed(range(0, self.n_T)):
            ...
            noise_pred, _ = self.nn_model(x_t, _c_gen, t.float() / self.n_T, sr_latents)
...
def update_ema(ema_vit, vit, decay=0.999):
    """Update EMA weights."""
    with torch.no_grad():
        model_params = dict(vit.named_parameters())
        ema_params = dict(ema_vit.named_parameters())

        for name, param in ema_params.items():
            if name in model_params:
                ema_param = param.data
                model_param = model_params[name].data
...
def training(args):
    ...
    save_dir = './output_old/'+'JEPA_1_timestep/' + dataset+'/'+model+ '/' + space + '/'+ label+ '/'+ experiment+'/'
    ...
    dit_transf = VisionTransformerTime(input_size=8, depth=12, hidden_size=384,
                 patch_size=1, num_heads=6, num_concepts=3, 
                 in_channels=128, learn_sigma=False, return_latents=sr_latents)
    ema_predictor = VisionTransformerTime(input_size=8, depth=12, hidden_size=384,
                 patch_size=1, num_heads=6, num_concepts=3, 
                 in_channels=128, learn_sigma=False, return_latents=sr_latents)
    ...
    ddpm = DDPM(nn_model=dit_transf, betas=(lrate, 0.02), n_T=n_T, device=device, drop_prob=0.1, n_classes=n_classes)
    ddpm.to(device)
    ...
    for ep in range(n_epoch):
        ...
        for x, c in pbar:
            ...
            loss, pred_latents = ddpm(emb, _c, sr_latents)
            t_1 = torch.tensor([1.0]).to(device)
            t_1 = t_1.repeat(emb.shape[0], 1, 1, 1)
            with torch.no_grad():
                _, post_pred_latents = ema_predictor(x= emb, t = t_1/n_T ,y = _c, return_latents = sr_latents)
                #print("post_pred_latents", post_pred_latents)
            
            sr_loss = 0
            for i in range(len(pred_latents)):
                sr_loss += F.mse_loss(pred_latents[i], post_pred_latents[i].detach())
            sr_loss /= len(pred_latents)
            loss += sr_loss
            ...
            update_ema(ema_predictor ,ddpm.nn_model, decay=0.999)
        ddpm.eval()
        with torch.no_grad():
            ...
            for test_config in configs["test"]: 
                for test_x, test_c in test_dataloaders[test_config]:
                    ...
                    test_loss, _ = ddpm(emb_test, _test_c, sr_latents)
            ...
            if (ep + 1) % 100 == 0 or ep >= (n_epoch - 5): 
                for test_config in output_configs: 
                    ...
                    if scheduler=="DDIM":
                        x_tok, x_gen_store = ddpm.sample_ddim(n_sample, c_gen, (in_channels ,8, 8), device, sr_latents=sr_latents)
                        ...
                        x_tok, x_gen_store = ddpm.sample(n_sample, c_gen, (in_channels ,8, 8), device,sr_latents=sr_latents ,guide_w=0.0)
                        x_gen = vqgan.decode(x_tok[...,1:,1:])
                        x_gen = x_gen[0] if isinstance(x_gen, tuple) else x_gen  # if dino loss is included
                        x_gen = (x_gen + 1)/2
